...
enum class Il2CppProfileFileIOKind { IL2CPP_PROFILE_FILEIO_WRITE = 0, IL2CPP_PROFILE_FILEIO_READ };
...
enum class Il2CppTypeEnum {
  ...
  IL2CPP_TYPE_VAR = 0x13, /* Generic parameter in a generic type definition, represented as number (compressed unsigned
                             integer) number */
...
static inline uint32_t GetDecodedMethodIndex(EncodedMethodIndex index) { return index & 0x1FFFFFFFU; }
...
enum class Il2CppRuntimeUnhandledExceptionPolicy { IL2CPP_UNHANDLED_POLICY_LEGACY, IL2CPP_UNHANDLED_POLICY_CURRENT };
...
struct Il2CppGenericClass {
  ...
  Il2CppGenericContext
               context; /* a context that contains the type instantiation doesn't contain any method instantiation */
...
enum class Il2CppCharSet { CHARSET_ANSI, CHARSET_UNICODE };
...
struct MethodInfo {
  ...
  /* note, when is_generic == true and is_inflated == true the method represents an uninflated generic method on an
   * inflated type. */
  union {
...
typedef struct Il2CppThread {
  ...
  int                 abort_state_handle;
  uint64_t tid; /* This is accessed as a gsize in the code (so it can hold a 64bit pointer on systems that need it), but
                   needs to reserve 64 bits of space on all machines as it corresponds to a field in managed code */
  void         *start_notify;
  void         *stack_ptr;
  void        **static_data;
  void         *jit_data;
  ...
  Il2CppThread *GetInternalThread() { return this; }

#else
  Il2CppInternalThread *GetInternalThread() const { return internal_thread; }
...
#define IL2CPP_CHECK_ARG_NULL(arg)                                                         \
  do {                                                                                     \
    if (arg == NULL) {                                                                     \
