...
std::vector<ProcMap> getAllMaps() {
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    sscanf(line, "%llx-%llx %s %llx %s %lu %s", &map.startAddress, &map.endAddress, perms, &map.offset, dev, &map.inode,
           pathname);
...
ProcMap getElfBaseMap(const std::string &name) {
  ...
  for (auto &it : maps) {
    ...
    if (rt == 0 || !info.dli_fname || !info.dli_fbase || it.startAddress != (uintptr_t)info.dli_fbase) continue;
...
Memory_Status memWrite(void *address, const void *buffer, size_t len) {
  ...
  if (kret != KERN_SUCCESS) {
    KITTY_LOGE("memWrite err vm_protect(page: %p, len: %zu, prot: %d) COW failed - kerror(%d).", (void *)page_start,
               page_len, page_info.protection, kret);
    return KMS_ERR_PROT;
  }
  ...
  if (kret != KERN_SUCCESS) {
    KITTY_LOGE("memWrite err vm_protect(page: %p, len: %zu, prot: %d) restore failed - kerror(%d).", (void *)page_start,
               page_len, page_info.protection, kret);
    return KMS_ERR_PROT;
  }
...
MemoryFileInfo getBaseInfo() {
  ...
  for (uint32_t i = 0; i < imageCount; i++) {
    ...
    // first executable
    if (exeIdx == -1) exeIdx = i;

    const char *name = _dyld_get_image_name(i);
    if (!name || strlen(name) != strlen(exeBuf.data()) || strcmp(name, exeBuf.data()) != 0) continue;
...
MemoryFileInfo getMemoryFileInfo(const std::string &fileName) {
  ...
  if (fileName.empty()) return _info;
  ...
  for (uint32_t i = 0; i < imageCount; i++) {
    ...
    if (!KittyUtils::String::EndsWith(fullpath, fileName)) continue;
...
uintptr_t findSymbol(const KittyMemory::MemoryFileInfo &info, const std::string &symbol) {
  if (!info.header || !info.address || symbol.empty()) return 0;
  ...
  if (!linkedit_segment_cmd || !symtab_cmd) return 0;
  ...
  *(uintptr_t *)&symtab   = (linkedit_base + symtab_cmd->symoff);
  ...
  for (uint32_t i = 0; i < symtab_cmd->nsyms; i++) {
    ...
    if (curr_sym_str.empty() || curr_sym_str != symbol) continue;
