...
inline Vector3 Quaternion::Up(Quaternion q) { return q * Vector3::Up(); }

inline Vector3 Quaternion::Down(Quaternion q) { return q * Vector3::Down(); }

inline Vector3 Quaternion::Left(Quaternion q) { return q * Vector3::Left(); }

inline Vector3 Quaternion::Right(Quaternion q) { return q * Vector3::Right(); }

inline Vector3 Quaternion::Forward(Quaternion q) { return q * Vector3::Forward(); }

inline Vector3 Quaternion::Back(Quaternion q) { return q * Vector3::Backward(); }
...
Quaternion Quaternion::FromEuler(Vector3 rotation) { return FromEuler(rotation.x, rotation.y, rotation.z); }
...
Quaternion Quaternion::LookRotation(Vector3 forward) { return LookRotation(forward, Vector3(0, 1, 0)); }
...
Quaternion Quaternion::LookRotation(Vector3 forward, Vector3 upwards) {
  ...
  if (1 - fabs(Vector3::Dot(forward, upwards)) < SMALL_FLOAT) return FromToRotation(Vector3::Forward(), forward);
...
float Quaternion::Norm(Quaternion rotation) {
  return sqrt(rotation.x * rotation.x + rotation.y * rotation.y + rotation.z * rotation.z + rotation.w * rotation.w);
}

Quaternion Quaternion::Normalized(Quaternion rotation) { return rotation / Norm(rotation); }
...
void Quaternion::ToAngleAxis(Quaternion rotation, float& angle, Vector3& axis) {
  if (rotation.w > 1) rotation = Normalized(rotation);
...
bool operator!=(const Quaternion lhs, const Quaternion rhs) { return !(lhs == rhs); }

std::string to_string(Quaternion a) {
  return to_string(a.x) + std::string(", ") + to_string(a.y) + std::string(", ") + to_string(a.z) + std::string(", ") +
         to_string(a.w);
}
